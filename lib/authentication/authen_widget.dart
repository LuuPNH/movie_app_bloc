...
class _AuthenWidgetState extends TeqWidgetState<AuthenBloc, AuthenWidget>
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
        providers: [BlocProvider(create: (context) => bloc..add(AuthenStartedEvent()))],
  ...
  Widget _buildbody(BuildContext context, AuthenState state) {
    ...
    return BlocBuilder<AuthenBloc, AuthenState>(builder: (context, state) {
      if (state.authenFailure) {
        ...
        return Container(color: Colors.green, child: Center(child: IconButton(icon: Icon(Icons.air), onPressed: () { BlocProvider.of<AuthenBloc>(context).add(AuthenLogoutEvent());})),);
      } else {
